function y = Spacecraft_Trajectory_OptimizationC2(x)
...
function xl = get_xl(~)
    xl = [-1000;3;0;0;100;100;30;400;800;0.01;0.01;0.01;0.01;0.01;1.05;1.05;1.15;1.7;-pi;-pi;-pi;-pi];
end

function xu = get_xu(~)
    xu = [0;5;1;1;400;500;300;1600;2200;0.9;0.9;0.9;0.9;0.9;6;6;6.5;291;pi;pi;pi;pi];
end

function J = mga_dsm(t)
    sequence = [3,2,2,3,5,6];
    problem.objective.type = 'total DV rndv';
    ...
    % Define gravitational constants for the solar system bodies
    mu(1) = 22321;          % Mercury
    mu(2) = 324860;         % Venus
    ...
    % Define radii of the planets (in km)
    RPL(1) = 2440; % Mercury
    RPL(2) = 6052; % Venus
    RPL(3) = 6378; % Earth
    RPL(4) = 3397; % Mars
    ...
    for i=1:N-2
        ...
        e=1+rp(i)/muvec(i+1)*v_rel_in'*v_rel_in; %#ok<MHERM>
    ...
    DVrel=norm(v(:,N)-v_sc_pl_in(:,N)); % Relative velocity at the target planet
    ...
    switch problem.objective.type
        ...
        case 'time to AUs'
            ...
            e=1+rp(N-1)/muvec(N)*v_rel_in'*v_rel_in; %#ok<MHERM>
            beta_rot=2*asin(1/e);              %velocity rotation
            ix=v_rel_in/norm(v_rel_in);
...
function t = time2distance(r0, v0, rtarget)
    ...
    if r0norm < rtarget
        ...
        if e < 1
            ra = a * (1 + e);
            if rtarget > ra
                t = NaN; % Target distance is unreachable.
            else
                % Find the anomaly where the target distance is reached.
                ni = acos((p/rtarget - 1) / e); % in the range 0 to pi
